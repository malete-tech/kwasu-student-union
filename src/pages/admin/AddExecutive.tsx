...
const AddExecutive: React.FC = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      toast.error("Failed to add executive profile.");
    } finally {
    ...
    <>
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-3xl font-bold tracking-tight text-brand-700">New Profile</h2>
            <p className="text-muted-foreground mt-1">Onboard a new member to the executive councils.</p>
          </div>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-12">
            ...
            <div className="grid gap-8 md:grid-cols-3">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">The member's basic information and council placement.</p>
              </div>
            ...
            <div className="grid gap-8 md:grid-cols-3">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">Upload a photo and set the duration of their term.</p>
              </div>
            ...
            <div className="flex pt-6">
              <Button type="submit" className="w-full sm:w-auto px-10 h-14 bg-brand-700 hover:bg-brand-800 text-white rounded-2xl shadow-xl hover:shadow-2xl transition-all text-lg font-bold" disabled={isSubmitting}>
                {isSubmitting ? <><Loader2 className="mr-3 h-5 w-5 animate-spin" /> Adding...</> : "Add Executive"}
              </Button>
            </div>
          </form>
        </Form>
